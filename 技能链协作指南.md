...
**何时调用：** 当需要生成流程图时，优先调用 `/diagram-design`，不可用时回退到 Mermaid。

**调用方与场景：**

| 调用方 | 调用场景 | diagram 类型 | 输出路径 |
|--------|----------|-------------|----------|
| logic-list-spec Extract | 页面操作流程图、状态流转图 | flowchart / state-machine | `doc/V{版本}/diagrams/` |
| prd-auto-generator | 全流程图、校验流程图、状态流转图 | flowchart / state-machine | `PRD/diagrams/` |
...
**安装地址：** https://github.com/cathrynlavery/diagram-design
...
| **logic-list-spec Extract** | 原型HTML源码 | 正式版业务逻辑清单（含截图引用 + diagram-design 流程图） |
